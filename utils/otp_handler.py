...
class OTPHandler:
    ...
    async def send_otp(
        db: AsyncSession,
        user: User, # User object contains customer_id
        otp_type: str,
        ...
        new_otp_record = OtpVerification(
            user_id=user.customer_id, # Changed from user.id to user.customer_id
    ...
    async def verify_otp(db: AsyncSession, user_customer_id: str, otp_code: str) -> Optional[OtpVerification]:
        ...
        Updates OTP record status.
        """
        result = await db.execute(
            select(OtpVerification)
            .where(
                OtpVerification.user_id == user_customer_id, # Changed from user_id to user_customer_id
